...
def market_ok(d):
    ...
    i = market_df.index.get_loc(d)
    if i < MARKET_MA:
        return False

    close = market_df["Close"].iloc[i]
    ma75 = market_df["MA75"].iloc[i]
    ma25_now = market_df["MA25"].iloc[i]
    ma25_prev = market_df["MA25"].iloc[i-1]

    # 緩い条件
    return (close > ma75) or (ma25_now > ma25_prev)
...
market_df = load_data(MARKET_TICKER)
