...
def run():
    ...
    candidates = safe_read_csv(CANDIDATES_FILE, CANDIDATE_COLS)
    ...
    # candidate count this week
    week_candidates = pd.DataFrame(columns=CANDIDATE_COLS)

    if len(candidates) > 0:
        candidates["signal_date"] = pd.to_datetime(candidates["signal_date"], errors="coerce")

        week_candidates = candidates[
            (candidates["signal_date"] >= pd.Timestamp(start))
            & (candidates["signal_date"] <= pd.Timestamp(end))
        ]

    candidate_text = "none"

    if len(week_candidates) > 0:
        candidate_text = week_candidates["ticker"].value_counts().to_string()

    if len(trades) == 0:
        msg = f"""📊 v624 WEEKLY REPORT
