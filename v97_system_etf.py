...
def load_data(ticker: str) -> pd.DataFrame:
    ...
    for col in ["Close", "Volume"]:
        if col not in df.columns:
            print(f"{ticker}: missing column {col}, columns={list(df.columns)}")
            return pd.DataFrame()
...
def get_latest_cash_value() -> float | None:
    eq = load_equity_df()
    if eq.empty or "cash" not in eq.columns:
        return None

    s = pd.to_numeric(eq["cash"], errors="coerce").dropna()
    if s.empty:
        return None
...
def calc_starting_cash_for_signal_date(signal_date: pd.Timestamp) -> float:
    ...
    latest_cash = get_latest_cash_value()
    ...
    return latest_cash + extra_flow


# =========================
# HOLD DAYS
# =========================
def calc_bars_passed(df: pd.DataFrame, entry_date: pd.Timestamp, signal_date: pd.Timestamp) -> int:
...
# CANDIDATES
# =========================
def build_candidates_with_diagnostics(signal_date: pd.Timestamp, pos: pd.DataFrame, data_cache: dict[str, pd.DataFrame]):
    candidates = []
    stats = {
        "total": 0,
        "has_data": 0,
    ...
    held = set(pos["ticker"].values) if ("ticker" in pos.columns and not pos.empty) else set()
...
def main():
    ...
    # ENTRY
    # =====================
    candidates, filter_stats = build_candidates_with_diagnostics(signal_date, pos, data_cache)
