# v502_morning_star_walkforward_test.py
# -*- coding: utf-8 -*-
...
def is_morning_star(df, i):
    ...
    first_bearish = c1 < o1
    first_long = body1 / o1 >= LONG_BODY_PCT

    second_small = body2 <= body1 * SMALL_BODY_RATIO
    second_low_area = max(o2, c2) <= (o1 + c1) / 2

    third_bullish = c3 > o3
    third_long = body3 / o3 >= THIRD_BODY_PCT

    recovery_level = c1 + body1 * RECOVERY_RATIO
    recovered = c3 >= recovery_level
    ...
    for d in dates:
        ...
        if len(pos) < MAX_POSITIONS:
            for t, df in data.items():
                ...
                if i < max(MA_LONG, PULLBACK_DAYS) + 2:
                    continue
