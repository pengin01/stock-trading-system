# v509_engulfing_only_filtered.py
...
def is_bullish_engulfing(df, i):
    ...
    engulfing = open_ <= prev_close and close >= prev_open

    if not (prev_bearish and today_bullish and engulfing):
        return False

    # 軽い強化条件：
    # 当日終値が2日前終値も上回る
    # 弱い戻りの包み足を減らす
    if close <= df["Close"].iloc[i - 2]:
        return False
...
def print_result(label, start, end):
    ...
    if trades:
        ...
        print(f"  win_rate  : {(tdf['pnl'] > 0).mean()}")
        print(f"  avg_return: {tdf['return'].mean()}")
...
print("\n=== WALK FORWARD : ENGULFING ONLY + FILTER v509 ===")
