...
OUTPUT_ROOT_PREFIX = "backtest_results_stock_v1_stable"
...
def create_output_dir(prefix: str) -> Path:
    ...
    out_dir = Path(f"{prefix}")
    out_dir.mkdir(parents=True, exist_ok=True)
    return out_dir


def print_global_settings():
    print("=== STOCK BACKTEST V1 STABLE ===")
...
def make_signal_diagnostics(
    ...
    for ticker, df in data_map.items():
        row = {
            ...
            "market_count": int(df["Market_Trend"].sum()),
...
def main():
    ...
    for idx, params in enumerate(param_combinations, start=1):
        ...
        feature_map = build_features_for_params(
            base_map=base_map,
            params=params,
        )
